import { c as createComponent } from './astro-component_tAHmzAVm.mjs';
import 'piccolore';
import { n as createRenderInstruction, m as maybeRenderHead, h as addAttribute, r as renderTemplate, o as renderHead, p as renderComponent, q as renderSlot } from './entrypoint_9FQd-zhG.mjs';
import 'clsx';
...
async function getCollectionData() {
  const STRAPI_URL2 = "https://nice-beef-8685fc168d.strapiapp.com";
  const response = await fetch(
    `${STRAPI_URL2}/api/collection?populate[sections][populate]=*`
  );
  const json = await response.json();
  return json.data;
}
...
async function getSectionModuleData() {
  const STRAPI_URL2 = "https://nice-beef-8685fc168d.strapiapp.com";
  const response = await fetch(
    `${STRAPI_URL2}/api/collection?populate[sections][on][block.section-module][populate][products][populate]=*`
  );
  const json = await response.json();
  return json.data;
}
